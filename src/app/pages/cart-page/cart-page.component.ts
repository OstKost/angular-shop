...
import {ChangeCartItemSubmit, GetCartSubmit, RemoveCartItemSubmit} from '../../store/actions/cart.actions';
...
export class CartPageComponent implements OnInit {
  ...
  cart$ = this.store.pipe(select(selectCartList), map((items) => {
    this.cart = items;
    return items;
  }));
  cart: CartItem[] = [];
  ...
  submitHandler(values): void {
    this.result = JSON.stringify({
      ...
      items: this.cart.map(({id, count, total, product}) => ({
        id, count, total, price: product.price
      })),
    }, null, 2);
  }
}
